...
def load_citation_metadata():
    ...
    if not citation_file.exists():
        ...
        return {'version': 'v0.1', 'codename': 'Blantyre', 'title': 'TEP-RBH'}
    
    try:
        if yaml:
            with open(citation_file, 'r') as f:
                data = yaml.safe_load(f)
            version_str = str(data.get('version', 'v0.1'))
        else:
        ...
        if match:
            ...
            codename = match.group(2) or 'Blantyre'
        else:
...
def copy_pdf_to_docs(source_pdf: Path, docs_dir: Path):
    ...
    # Primary PDF name (matches manuscript format: 7-TEP-RBH-v0.2-Blantyre.pdf)
    target_name = f"7-TEP-RBH-{version_str}.pdf"
